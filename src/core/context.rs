...
#[cfg(windows)]
use windows::Win32::Graphics::Direct3D11::{ID3D11Texture2D, ID3D11Device};
...
impl AMFContext {
    ...
    #[cfg(windows)]
    pub fn init_dx11(&self, device: ID3D11Device, dx_version: AMFDXVersion) -> Result<(), AMFError> {
        use windows::core::Interface;
        
        unsafe { (self.vtable().init_dx11)(self.as_raw(), device.as_raw(), dx_version).into_error() }
    }

    /// # Safety
    /// `device` is not null
    /// `device` is ID3D11Device
    pub unsafe fn init_dx11_raw(&self, device: *mut c_void, dx_version: AMFDXVersion) -> Result<(), AMFError> {
